...
def TransformacionBilineal2(A,Bx,By,ImgO,ImgR,height1,width1,height2,width2):
    ...
    for i in range(height1,height2):
        for j in range(width1,width2):
            ...
            x=round(sum(vector*Solutions1))
            Solutions2=np.array(Solutions2)
            y=round(sum(vector*Solutions2))
            x=int(x)#+0.5)
            y=int(y)#+0.5)
            ...
            for y1 in range(2):
                for x1 in range(2):
                    ...
                    if(x+x1<width):
                        if(y+y1<height):
                            if(ImgR[y+y1,x+x1][0]==0 and ImgR[y+y1,x+x1][1]==0 and ImgR[y+y1,x+x1][2]==0):
                                #print("aqui", x1,y1)
                                ImgR[y+y1,x+x1]=ImgO[i,j]
                        if(y-y1>height):
                            if(ImgR[y-y1,x+x1][0]==0 and ImgR[y-y1,x+x1][1]==0 and ImgR[y-y1,x+x1][2]==0):
                                    #print("aqui", x1,y1)
                                ImgR[y-y1,x+x1]=ImgO[i,j]
...
def TransformacionBilineal1(A,Bx,By,ImgO,ImgR,height1,width1,height2,width2):
    Solutions2=np.linalg.solve(A,By)
    Solutions1=np.linalg.solve(A,Bx)#cramer(A,B)
    for i in range(height1,height2):
        for j in range(width1,width2):
...
for i in range(2):
    for j in range(2):
        ...
        Bx=np.array(Bx)
        By=np.array(By)
        TransformacionBilineal2(A,Bx,By,img,img2,Y[i],X[j],Y[i+1],X[j+1])
            #mostrar(img2)
mostrar(img2)
...
for i in range(2):
    for j in range(2):
        ...
        Bx=np.array(Bx)
        By=np.array(By)
        TransformacionBilineal1(A,Bx,By,img,img2,Y[i],X[j],Y[i+1],X[j+1])
            #mostrar(img2)
mostrar(img2)
    #break
